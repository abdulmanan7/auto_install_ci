...
class Installer
{
	...
	function _save_user()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$config_data['site']=$_POST['site_name'];
	...
	function _save_ion_auth_settings_to_file($data)
	{
		...
		$conf .="\$config['site_title']                 = '".$data['site']."';\n";
		$conf .="\$config['admin_email']                = '".$data['email']."';\n";
		...
		$conf .="\$config['message_start_delimiter'] = '<p>';\n\n";
		$conf .="\$config['message_end_delimiter']   = '</p>';\n\n";
		$conf .="\$config['error_start_delimiter']   = '<p>';\n\n";
